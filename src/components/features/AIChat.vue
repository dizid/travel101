...
onMounted(async () => {
  if (isPro.value) {
    ...
    if (hasHistory && conversationHistory.value.length > 0) {
      showResumedBanner.value = true
      // Auto-dismiss after 3s
      setTimeout(() => {
        showResumedBanner.value = false
      }, 3000)
    }
  }
})

// Watch for Pro status changes
watch(isPro, async (newVal) => {
  if (newVal && !conversationLoaded.value) {
    loadingHistory.value = true
    await loadConversation('general')
    loadingHistory.value = false
  }
})
  ...
  <div class="flex flex-col h-full bg-white rounded-2xl shadow-thai overflow-hidden">
    ...
    <div class="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-primary-500 to-primary-600 text-white">
      ...
      <div class="flex items-center gap-1">
        <button
          v-if="isPro && conversationHistory.length > 0"
    ...
    >
      <div
        v-if="showResumedBanner"
        class="px-4 py-2 bg-green-50 border-b border-green-200 text-green-700 text-sm text-center"
      >
    ...
    >
      ...
      <div v-if="loadingHistory" class="text-center py-8">
        <LoadingOutlined class="text-3xl text-primary-500 animate-spin mb-4" />
        <p class="text-gray-500 text-sm">Loading your conversation...</p>
      </div>

      <!-- Welcome message -->
      <div v-else-if="conversationHistory.length === 0" class="text-center py-8">
